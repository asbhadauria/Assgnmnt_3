 ...
 //to link it to the next and the previous node
	...
	public:
		...
		Node() {				// Default constructor for Node
			...
			next = NULL;		// and points previous and next to
			prev = NULL;		// NULL.
		}
};
...
class CLList {
 ...
 //Circular linked list class. Dummy pointer stores the address
...
Node* CLList::getNode(int i) {
 ...
 // Returns a pointer to the i-th node from dummy.
...
int CLList::insertAt(int i, int d) {
 ...
 //Inserts a new node at the i-th position. Numbering starts from 1.
...
void CLList::remove() {
 ...
 // Removes the element to which dummy's previous points.
